-- Fix database schema issues without creating conflicting primary keys

-- First check if tenant_id is already primary key
DO $$ 
BEGIN
    -- Drop the existing primary key on tenant_id if it exists and create new one on id
    IF EXISTS (SELECT 1 FROM information_schema.table_constraints 
               WHERE table_name = 'tenants' AND constraint_type = 'PRIMARY KEY') THEN
        -- Add id column if it doesn't exist (without making it primary key yet)
        IF NOT EXISTS (SELECT 1 FROM information_schema.columns 
                       WHERE table_name = 'tenants' AND column_name = 'id') THEN
            ALTER TABLE public.tenants ADD COLUMN id uuid DEFAULT gen_random_uuid();
        END IF;
    END IF;
END $$;
